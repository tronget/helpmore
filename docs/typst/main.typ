...
- F1.1 Система должна предоставлять возможность входа по логину и паролю.
- F1.2 Система должна предоставлять возможность подразделять пользователей на роли: пользователь (может быть заказчиком и исполнителем), модератор, администратор.

== Функциональные требования (авторизованный пользователь)
- F2.1 Система должна предоставлять возможность просмотра открытых предложений и заказов.
- F2.2 Система должна предоставлять возможность просмотра профилей пользователей (ФИО, bio, контакты, рейтинг и отзывы, количество выполненных заказов).
- F2.3 Система должна предоставлять возможность редактирования личного профиля (bio, контакты).
- F2.4 Система должна предоставлять возможность создания и публикации заказов (название, описание, сроки, стоимость/бартер).
- F2.5 Система должна предоставлять возможность создания публикации предложений услуг (название, описание, стоимость).
- F2.6 Система должна предоставлять возможность поиска и фильтрации открытых заказов (категория, сроки, стоимость).
- F2.7 Система должна предоставлять возможность поиска и фильтрации открытых предложений (категория, рейтинг, стоимость).
- F2.8 Система должна предоставлять возможность отклика на заказы и связи с заказчиками.
- F2.9 Система должна предоставлять возможность отклика на предложения и связи с предоставителями.
- F2.10 Система должна предоставлять возможность ведения чата с другими пользователями (обсуждение условий, обмен изображениями).
- F2.11 Система должна предоставлять возможность оставления оценок и текстовых отзывов после выполнения услуги.
- F2.12 Система должна предоставлять возможность просмотра рейтинга и отзывов всех пользователей.
- F2.13 Система должна предоставлять возможность просмотра личной истории заказов и оказанных предложений.
- F2.14 Система должна предоставлять возможность отметки интересных заказов/предложений (избранное).
- F2.15 Система должна предоставлять возможность подачи жалоб на некорректное поведение или нарушение правил.

== Функциональные требования (модератор)
- F3.1 Система должна предоставлять возможность удаления некорректных заказов/предложений.
- F3.2 Система должна предоставлять возможность рассмотрения жалоб пользователей.
- F3.3 Система должна предоставлять возможность выдачи временной блокировки пользователей при нарушении правил.

#text(style: "italic")[\*Модератор наследует возможности простого пользователя.]


== Функциональные требования (администратор)

- F4.1 Система должна предоставлять возможность управления категориями услуг (создание, редактирование, удаление).
- F4.2 Система должна предоставлять возможность управления ролями пользователей (назначение модераторов, снятие прав).
- F4.3 Система должна предоставлять возможность удаления или восстановления учетных записей пользователей.


#text(style: "italic")[\*Администратор наследует возможности модератора.]
...
- U1.2 Корректная работа в современных браузерах: Chrome (79+), Safari (11+), Mozilla (70+), Яндекс Браузер (21+).
- U1.3 Интуитивный интерфейс и удобная навигация; нахождение нужного контента менее чем за 45 секунд.
- U1.4 Доступность: ARIA-атрибуты.
- U1.5 Мультиязычность интерфейса; выбор языка через настройки аккаунта.
- U1.6 Унификация навигации (стабильные позиции главного меню, поиска и основных кнопок).

=== R2: Надежность

- R2.1 Доступность не менее 99,999%; в случае критических ошибок — только кратковременное ограничение функционала.
- R2.2 Индикатор процесса при загрузке файлов или больших данных.
- R2.3 Единый учёт отзывов, оценок, количества выполненных заказов (консистентная БД).
- R2.4 Мониторинг (например, Grafana).

=== P3: Производительность

- P3.1 Первые результаты поиска ≤ 2 секунды (с использованием Hibernate search).
- P3.2 Обработка до 100 одновременных запросов к БД.
- P3.3 Пропускная способность: ~10000 просмотров услуг и ~1000 публикаций в день; сеть до 100 Мбит/с.
- P3.4 Загрузка любой страницы за 1–2 секунды при 10 Мбит/с.
- P3.5 Эффективное кэширование (Redis) для часто запрашиваемых и редко изменяемых данных (личный аккаунт, справочники, результаты поиска/фильтрации).

=== S4: Поддерживаемость

- S4.1 Сервисно-ориентированная архитектура (SOA)
- S4.2 API-first, публичные и внутренние API.
- S4.3 Контейнеризация (Docker).
- S4.4 RESTful API (JSON), стандартные HTTP-методы (GET, POST, PUT, DELETE).

- S4.5 Регулярные code review и рефакторинги.
- S4.6 Документация к внутреннему и внешнему API (OpenAPI).
- S4.7 Слабая привязка к провайдеру через абстракции Docker.
- S4.8 Мониторинг и алертинг (Grafana).
- S4.9 Интернационализация интерфейса (i18n, напр. Globalize.js) для поддержки трёх языков.
