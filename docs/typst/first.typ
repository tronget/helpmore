...
- дата публикации и актуальность.
...
- Модераторы — пользователи с расширенными правами, которые проверяют корректность информации об услугах, следят за соблюдением правил и контролируют взаимодействие участников.
- Администраторы — пользователи с полным доступом к системе: управление категориями услуг, настройка механизмов поиска и фильтрации, контроль финансовых транзакций, поддержка пользователей.
...
- Бытовые услуги — помощь с переездом, доставкой, ремонтом, настройкой техники, покупками.
- Карьерные услуги — консультации по стажировкам, подготовка резюме, тренировка собеседований.
- Социальные и информационные услуги — советы по адаптации в университете, обмен опытом участия в мероприятиях, волонтёрских программах или студенческих клубах.
...
- *Заказ* — это запрос на оказание услуги, содержащий описание задачи, сроки, стоимость и пожелания к исполнителю.
...
- стоимости и срокам выполнения.

=== 7. Коммуникация

Зарегистрированные пользователи общаются через встроенный чат, обсуждают детали заказа, делятся файлами (документы, код, отчёты), оставляют комментарии.
...
- F1.1 Система должна предоставлять возможность входа по ITMO.ID, по e-mail и паролю; подтверждение статуса студента по адресу niuitmo.ru / itmo.ru.
- F1.2 Система должна предоставлять возможность восстановления доступа к аккаунту по e-mail.
- F1.3 Система должна предоставлять возможность подразделять пользователей на роли: пользователь (может быть заказчиком и исполнителем), модератор, администратор.

== Функциональные требования (авторизованный пользователь)

- F2.1 Просмотр открытых предложений и заказов.
- F2.2 Просмотр профилей пользователей (ФИО, bio, контакты, рейтинг и отзывы, количество выполненных заказов).
- F2.3 Редактирование личного профиля (bio, контакты).
- F2.4 Создание и публикация заказов (описание, сроки, стоимость/бартер).
- F2.5 Создание и публикация предложений услуг (описание, стоимость).
- F2.6 Поиск и фильтрация открытых заказов (категория, сроки, стоимость).
- F2.7 Поиск и фильтрация открытых предложений (категория, рейтинг, стоимость).
- F2.8 Отклик на заказы и связь с заказчиками.
- F2.9 Отклик на заказы и связь с заказчиками. 
- F2.10 Ведение чата с другими пользователями (обсуждение условий, обмен файлами).
- F2.11 Оставление оценок и текстовых отзывов после выполнения услуги.
- F2.12 Просмотр рейтинга и отзывов всех пользователей.
- F2.13 Просмотр личной истории заказов и оказанных услуг.
- F2.14 Отметка интересных заказов/предложений (избранное).
- F2.15 Подача жалоб на некорректное поведение или нарушение правил.

== Функциональные требования (модератор)

- F3.1 Сокрытие или удаление некорректных заказов/предложений.
- F3.2 Рассмотрение жалоб пользователей.
- F3.3 Временная блокировка пользователей при нарушении правил.
- F3.4 Перманентная блокировка пользователей при нарушении правил.
- F3.5 Отправка предупреждений пользователям.

== Функциональные требования (администратор)

- F4.1 Управление категориями услуг (создание, редактирование, удаление).
- F4.2 Управление ролями пользователей (назначение модераторов, снятие прав).
- F4.3 Управление глобальными настройками системы (фильтры, правила поиска, доступность функций).
- F4.5 Просмотр агрегированной статистики по активности пользователей, заказам и отзывам.
- F4.6 Удаление или восстановление учетных записей пользователей.
- F4.7 Модерация отзывов и оценок.

== Нефункциональные требования

=== U1: Удобство использования

- U1.1 Единый дизайн (единый UI-kit).
- U1.2 Адаптивная вёрстка (мобильные и десктопные устройства).
- U1.3 Корректная работа в современных браузерах: Chrome (79+), Safari (11+), Mozilla (70+), Яндекс Браузер (21+).
- U1.4 Интуитивный интерфейс и удобная навигация; нахождение нужного контента менее чем за 45 секунд.
- U1.5 Доступность: ARIA-атрибуты, режим высокой контрастности, рекомендации WAI.
- U1.6 Мультиязычность интерфейса; выбор языка через настройки аккаунта.
- U1.7 Контекстная справка при ошибках.
- U1.8 Унификация навигации (стабильные позиции главного меню, поиска и основных кнопок).

=== R2: Надежность

- R2.1 Доступность не менее 99,999%; в случае критических ошибок — только кратковременное ограничение функционала.
- R2.2 Индикатор прогресса при загрузке файлов или больших данных.
- R2.3 Единый учёт отзывов, оценок, количества выполненных заказов (консистентная БД).
- R2.4 Плановый MTBF ≥ 10 000 часов; мониторинг (например, Grafana).

=== P3: Производительность

- P3.1 Загрузка главной страницы ≤ 3 секунды при 10 Мбит/с.
- P3.2 Первые результаты поиска ≤ 1 секунды (например, Elasticsearch).
- P3.3 Обработка до 500 одновременных запросов к БД.
- P3.4 Пропускная способность: ~100000 просмотров услуг и ~100\0 публикаций в день; сеть до 1 Гбит/с.
- P3.5 Загрузка любой страницы за 1–2 секунды при 10 Мбит/с.
- P3.6 Восстановление критичных сервисов ≤ 5 минут; некритичных — ≤ 30 минут.
- P3.7 Эффективное кэширование (Redis) для часто запрашиваемых и редко изменяемых данных (личный аккаунт, справочники, результаты поиска/фильтрации).

=== S4: Поддерживаемость

- S4.1 Сервисно-ориентированная архитектура (SOA), обновление/масштабирование сервисов параллельно.
- S4.2 API-first, публичные и внутренние API.
- S4.3 Контейнеризация (Docker).
- S4.4 RESTful API (JSON/XML), стандартные HTTP-методы (GET, POST, PUT, DELETE), OAuth 2.0.
- S4.5 Соблюдение стандартов кодирования (линтеры: ESLint, spotless), регулярные code review и рефакторинги.
- S4.6 Документация к внутреннему API (OpenAPI).
- S4.7 Слабая привязка к провайдеру через абстракции Docker.
- S4.8 Мониторинг и алертинг (Grafana).
- S4.9 Интернационализация интерфейса (i18n, напр. Globalize.js) для поддержки десятков языков.
...
#table(
  ...
  [Прецедент], [U001],
  [ID], [1],
  [Краткое описание], [Пользователь авторизуется в системе через itmo.id],
  [Главный актор], [Пользователь с учетной записью в ИСУ ИТМО],
  [Второстепенные акторы], [Система, Сервис itmo.id],
  [Предусловия], [Пользователь имеет учетную запись в ИСУ ИТМО, но не авторизован в системе.],
  [Основной поток], [
    1. Пользователь нажимает кнопку "Войти" на главной странице HelpMore.
    2. Система перенаправляет пользователя на страницу авторизации сервиса itmo.id.
    3. Пользователь вводит свои логин и пароль от itmo.id. 
    4. Сервис itmo.id аутентифицирует пользователя и подтверждает его статус. 
    5. Система HelpMore получает от itmo.id минимальный набор данных. 
    6. Система проверяет, существует ли уже профиль для этого пользователя. Если нет – автоматически создает его на основе полученных данных. 
    7. Система аутентифицирует пользователя и перенаправляет его на главную страницу или страницу заполнения информации для профиля (навыки, описание).],
  [Альтернативный поток], [A1. Ошибка аутентификации в itmo.id (неверный логин/пароль). Сервис itmo.id отображает сообщение об],
)
...
#table(
  ...
  [Основной поток], [
    ...
    2. Система отображает форму с полями: категория (учебная/бытовая и т.д.), заголовок, подробное описание, желаемые сроки, бюджет (если есть). 
...
#table(
  ...
  [Альтернативный поток], [A1. Ничего не найдено. Система предлагает изменить параметры поиска.],
  [Постусловия], [Пользователь нашел подходящие предложения или запросы.],
)
== Прецедент U005
...
- Фронтенд (CSR) с адаптивным дизайном, интернационализацией (RU/EN).
